<!-- RoomSettingsPanel.vue: 방 폭·높이를 수정하는 보조 설정 패널 -->
<template>
    ...
    <div :style="styles.inputSection">
      ...
      <div :style="styles.inputGroup">
        ...
        <input
          type="number"
          :value="heightInput"
          @input="(e) => heightInput = (e.target as HTMLInputElement).value"
          :style="styles.input"
      ...
      <button @click="handleApply" :style="styles.button">적용</button>
    </div>
...
const emit = defineEmits<{
  roomChange: [room: RoomState];
}>();

const widthInput = ref(props.room.roomWidthMm.toString());
const heightInput = ref(props.room.roomHeightMm.toString());
...
const handleApply = () => {
  ...
  if (!heightValidation.isValid) {
    error.value = heightValidation.message || '잘못된 높이 값입니다.';
    return;
  }
